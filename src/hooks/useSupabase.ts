// src/hooks/useSupabase.ts - VERSÃO ATUALIZADA COM getProductById
...
export function useSupabase() {
  ...
  // ==========================
  // ✅ NOVA FUNÇÃO: getProductById
  // ==========================
  const getProductById = async (productId: string): Promise<ProductData | null> => {
    try {
  ...
  // ==========================
  // RENOVAÇÃO E CANCELAMENTO DE RESERVAS
  // ==========================
  const renovarReserva = async (reservaId: string, horas: number = 8) => {
    try {
      ...
      const { error: updateError } = await supabase
        .from('reservas')
        .update({
          fim_reserva: novoFimReserva.toISOString(),
          status: 'pendente',
  ...
  // ✅ FUNÇÃO RPC PARA RESERVAS
  const createReservationRPC = async (
    produtoId: string,
    usuarioId: string,
    lojaId: string,
    quantidade: number = 1,
  ...
  return {
    ...
    // Produtos (INCLUINDO getProductById)
    getProductById, // ✅ NOVA FUNÇÃO ADICIONADA
    createProduct,
    updateProduct,
    getProducts,
    getProductsSafe,
