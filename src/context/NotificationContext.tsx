...
export function NotificationProvider({ children }: { children: ReactNode }) {
  ...
  const refreshData = async () => {
    try {
      ...
      const [categoriesData, preferencesData, userNotifs] = await Promise.all([
        categoryService.getActiveCategories(),
        notificationService.getUserNotificationPreferences(),
        notificationService.getUserNotifications()
      ]);

      const safeCategories: NotificationCategory[] = Array.isArray(categoriesData)
        ? categoriesData.map((cat: any) => ({
            ...cat,
            description: cat.description || "Descrição não disponível",
            source: cat.source || "system"
          }))
        : [];

      const safeUserNotifications: ImportedUserNotification[] = Array.isArray(userNotifs)
        ? userNotifs.map((notif: any) => ({
            ...
            user_id: notif.user_id || "unknown",
            type: notif.type || "info",
            category_id: notif.category_id,
            loja_id: notif.loja_id,
      ...
      setUserNotifications(safeUserNotifications);

      const unread = safeUserNotifications.filter((notif) => !notif.is_read).length;
