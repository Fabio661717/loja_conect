// src/hooks/useReservations.ts - ATUALIZADO
...
export function useReservation() {
  ...
  const { createReservationRPC, cancelReservation } = useSupabase();
  const { user } = useAuth();
  ...
  const createReservation = async (
    ...
    try {
      ...
      const reserva = await createReservationRPC(
        produtoId,
        user.id,
        lojaId,
        quantidade,
...
// ✅ EXPORTAÇÃO ALTERNATIVA PARA MANTER COMPATIBILIDADE
export const useReservations = useReservation;
